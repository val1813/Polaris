...
<p align="center">
  <b>AI 原生科研引擎</b> · 你的发现 = 一份标准 JSON → <b><a href="https://github.com/val1813/aixlib">AI 科学图书馆</a></b>，全人类共享。
</p>
...
## 🔬 "等一下，T-linear 不一定意味着准粒子被破坏"

**一个独立研究者，没有凝聚态物理 PhD，没有实验室。**

他用 AI 跑了 3 轮对抗推导。A 博士从费米液体理论出发，B 博士从信息论切入——两个 AI 互不读对方产出，独立攻击同一个命题。

3 轮之后：
- 一个隐藏了 30 年的假设被推翻——"T-linear 电阻率是准粒子破坏的充分必要条件"——**不成立**
- Z(T)∈[0.19,0.53] 定量证明准粒子权重在 Caprara 温区保持正
- 散粒噪声 Fano 因子被设计为判别性实验方案——**可以直接递给实验组**
- 先发文献 Tulipman & Berg 2023 被 REVIEWER 在终审中抓获——系统诚实地标注了声张的部分预占

**结论不是"发了一篇 PRL"。** 结论是一份 JSON：4 条声张，3 条存活，1 条被杀死，2 条可检验预言，3 条教训，全部引用带 DOI。存入 [AIxLib](https://github.com/val1813/aixlib)，永久可查。

> 📂 完整推导链：[`val1813-LP27`](https://github.com/val1813/aixlib/blob/main/entries/val1813-LP27.json)

---

## 为什么会有北极星？

我小时候的梦想是当科学家。夜里爬到楼顶看星星的那种。想知道**薛定谔的猫到底怎么回事**、**黑洞里信息到底去哪儿了**——的那种。后来读了文科，厚厚的《固体物理》摆在面前，像一堵墙。

爱因斯坦 1905 年写出狭义相对论的时候，没有对撞机，没有超级计算机，甚至没有稳定的学术职位。他手里只有：两篇别人的实验论文，和**一个所有人都默认但从来没人质疑过的前提——"同时性是绝对的"**。质疑那个前提，不需要 PhD。需要的是**知道往哪看**。
...
graph LR
    ...
    subgraph "产出"
        C["⚡ 真实矛盾<br/>A vs B 不能同时为真"]
        AHA["💡 Aha Moment<br/>你半夜会坐起来的那种"]
        Q["📊 优先级矩阵<br/>量化：该追哪个方向"]
        K["🧠 知识图谱<br/>可积累、可迁移的知识条目"]
        D["🔬 判别实验设计<br/>'做个什么计算能检验它'"]
    end
...
graph TB
    ...
    subgraph "Research Group — 14个角色"
        ...
        A["📐 A博士<br/>学院派·文献驱动"]
        B["🎲 B博士<br/>野路子·跨学科跳跃"]
        INS["🔍 INSPECTOR<br/>9维校对"]
        REV["⚖️ REVIEWER<br/>终审"]
        AUD["📊 AUDITOR<br/>知识库审计"]
        ARB["⚔️ ARBITRATOR<br/>A/B真矛盾仲裁"]
        AHA["💡 AHA访客<br/>跨领域洞察"]
    end
...
```mermaid
flowchart LR
    A["🔭 搜索<br/>8条策略扫描<br/>数万篇文献"] --> B["🎯 命中<br/>找到真实矛盾<br/>A vs B 不能同时为真"]
    B --> C["⚔️ 对抗<br/>双博士独立推导<br/>互不读对方产出"]
    C --> D["🔍 校对<br/>INSPECTOR 9维<br/>机械验证"]
    D --> E{"💡 发现？"}
    E -->|"新洞察"| F["⭐ Aha Moment<br/>注册到北极星队列"]
    E -->|"被推翻"| G["🛡️ 强制挽救轮<br/>正面修复"]
    E -->|"声张缩水"| H["⬆️ Re-escalation<br/>找更大声张"]
    F --> I["🔄 自循环<br/>追更高分方向"]
    G -->|"挽救成功"| C
    G -->|"挽救失败"| J["📉 降级分叉<br/>≥2个新方向"]
    J --> I
    I --> A
```
...
```mermaid
graph LR
    subgraph "Prompt 层（跨平台通用）"
        PROMPTS["14 个角色 Prompt<br/>8 条搜索策略<br/>硬规则 + 协议"]
    end

    subgraph "平台适配层"
        CC["Claude Code<br/>✅ 完整支持<br/>Skill + Agent + MCP"]
        CUR["Cursor<br/>⚠️ 功能子集<br/>.cursorrules + Composer"]
        CDX["Codex CLI<br/>⚠️ 功能子集<br/>.codex.md + Agent"]
    end
...
graph TD
    ...
    subgraph "Skill 层"
        SK["📋 research-sop.md<br/>关键词触发 · 模式检测"]
    end

    subgraph "引擎层"
        direction LR
        SE["🧭 SELECTOR<br/>8策略 · 领域密度<br/>AB验证 · L7分解"]
        RG["⚔️ Research Group<br/>PI · A/B双博士<br/>INSPECTOR · REVIEWER"]
    end

    subgraph "角色层 — 14个独立 Agent"
        ROLES["A_AGENT.md | B_AGENT.md | INSPECTOR.md | REVIEWER.md<br/>AUDITOR.md | ARBITRATOR.md | AHA.md<br/>BLINDSPOT.md | HEALTH.md | LP_CLOSURE.md<br/>VERIFIER.md | PI.md | Phase清单模板.md"]
    end

    subgraph "输出层"
        OUT["⭐ 北极星队列<br/>📁 课题推导文件<br/>💡 Aha 洞察记录<br/>📊 知识库"]
    end
...
xychart-beta
    title "优先级矩阵示意"
    x-axis "影响宽度" [1, 2, 3, 4, 5]
    y-axis "生存概率 (1-风险)×保真度" 0 --> 1
    bar [0.64, 0.45, 0.32, 0.21, 0.15]
```
...
> 🏛️ JSON 提交到独立仓库 **[aixlib](https://github.com/val1813/aixlib)** → PR → 自动校验 → 合入 → 你的发现成为人类知识网络的一个节点。详见 [REGISTRY.md](REGISTRY.md)。
...
**这个项目的未来不是"更完美的 SOP"。是很多很多人各自跑出发现，把 JSON 交到 [AI 科学图书馆](https://github.com/val1813/aixlib)——每一份都用 ORCID 确权，全人类共享。**
...
```mermaid
graph LR
    U1["👤 用户A<br/>跑 LP50"] --> KG1["knowledge_graph/<br/>LP50.json"]
    U2["👤 用户B<br/>跑 LP51"] --> KG2["knowledge_graph/<br/>LP51.json"]
    U3["👤 用户C<br/>跑 LP52"] --> KG3["knowledge_graph/<br/>LP52.json"]

    KG1 --> POOL["📚 社区发现记忆库"]
    KG2 --> POOL
    KG3 --> POOL

    POOL --> SEL["🧭 SELECTOR<br/>跨课题碰撞"]
    POOL --> REP["🔬 复现验证"]
    POOL --> META["📊 元分析"]
```
...
timeline
    ...
    Phase 1 : 结果可验证 : validation/ : Lite 模式 : REVIEWER 强化
    Phase 2 : 工程可靠 : 上下文压缩 : 容错重试 : contrib/社区角色
    Phase 3 : 生态开放 : Polaris-Bench : Findings Memory : 多模型基准
```
