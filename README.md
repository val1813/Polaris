...
<p align="center">
  ...
  <sub>不是写论文的工具。是寻找 <b>Aha Moment</b> 的引擎。</sub>
</p>
...
> **不是替代科学家。是让每一个有科学直觉的人——无论文理——都能在 AI 的帮助下，找到那个"所有人都默认但可能错了"的前提，发起一次真正的理论突破。**
...
- 曾经抬头看过星星，想过"为什么"
- 相信 AI 可以改变"谁可以做科研"这个问题的答案
- 手头有某个领域的专业知识，愿意贡献给角色 Prompt
- 发现了一个协议漏洞或 SOP 执行卡点
- 想把整个系统翻译成英文
...
这个系统跑了几十个课题，踩出了一堆真问题。如果你恰好擅长其中某个方向，**非常需要你的帮助**：

| # | 问题 | 现象 | 需要的帮助 |
|---|------|------|-----------|
| **1** | **物理意义空洞化** | 部分课题陷入"用几何解释几何"的循环——推导在数学上自洽，但始终无法落到可观测的物理量上。数学自洽 ≠ 物理意义。 | 物理直觉强的人，帮忙设计"物理意义落地检查"——什么样的中间步骤说明推导真的在说物理而不只是数学？ |
| **2** | **Token 消耗过高** | 一轮完整 AB + INSPECTOR + 综合，14 个角色轮番上阵，上下文窗口轻松几千行。一个课题跑到底，token 账单可观。 | 懂 prompt 工程或 agent 架构的人，帮忙做两件事：<br>• 哪些协议步骤可以合并压缩？<br>• 能不能做增量上下文（只传 diff，不全量重传）？ |
| **3** | **检验阶段 AI 放水** | INSPECTOR 和 REVIEWER 偶尔会漏掉错误——不是故意，是注意力在长上下文里被稀释了。一个未被发现的量纲错误，会让后面所有推导建立在沙子上面。 | 懂代码验证或形式化方法的人，能不能把部分检验逻辑做成**确定性脚本**（量纲检查、代数展开、极限退化）而非依赖 AI 注意力？ |
| **4** | **声张持续缩水** | 出发时雄心勃勃——"这个矛盾打到根上能发 PRL"。三轮过后，声张范围从"所有材料"→"部分材料"→"特定参数范围"。最后变成了四区水平的经验关联。已有 Re-escalation 机制但触发太晚。 | 懂科研方法论的人，帮忙设计**早期声张缩水预警**——不等三轮，第一轮出现什么信号就该拉警报？ |

> 🎯 **如果你恰好是：** 凝聚态物理 PhD / 做形式化验证的 / 做 prompt 优化的 / 发过 PRL 的 / 审过 PRL 的——你的经验对这个系统来说是无价的。一句话的指点可能比十轮迭代更有用。
...
**PR 规范：** 说明改了哪个协议、为什么改、测试情况。涉及硬规则的 PR 附一个案例。

### 也欢迎分享你的成果

跑出了有意思的东西？不管是 Aha Moment 还是踩坑经验，放到 `examples/` 目录下。参考 `examples/README.md` 了解格式。
